    ...
    <x-slot name="content">
        <form>
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                ...
                @if(count($tiposAlimentos) > 0)
                    <div class="mb-4">
                        <label class="inline-block w-32 font-bold">Tipo alimento:</label>
                        <select name="tipo" wire:model="tipo" 
                            ...
                            @foreach($tiposAlimentos as $tipoAlimento)
                                <option value="{{ $tipoAlimento }}">{{ $tipoAlimento }}</option>
                            @endforeach
                        </select>
                    </div>
                @endif
